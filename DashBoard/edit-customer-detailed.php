...
if (isset($_POST['delete'])) {
    ...
    $stmt = mysqli_prepare($con, "DELETE FROM tblcustomers WHERE ID = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);

    if (mysqli_stmt_affected_rows($stmt) > 0) {
        $msg = "Customer detail has been deleted.";
    } else {
        $msg = "Something went wrong. Please try again.";
    }
						 ...
						 <p style="font-size:16px; color:red" align="center"> <?php if($msg){
							...
							<div class="form-group"> 
								<label for="exampleInputPassword1">ID Number</label>
								 <input type="text" id="CustomerId" name="CustomerId" class="form-control"  value="<?php  echo $row['ID'];?>" required="true">
							</div>
							...
							<?php } ?>
							<button type="submit" name="delete" class="btn btn-default">Delete</button> </form> 
							</div>
